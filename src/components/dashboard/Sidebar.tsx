...
import { ChevronDown, ChevronLeft, ChevronRight, ExternalLink, LucideIcon, X } from 'lucide-react';
        ...
        {[3, 6, 3, 1].map((count, g) => (
          <div key={g} className="space-y-1.5">
            ...
            {Array.from({ length: count }).map((_, i) => (
              <div key={i} className="h-8 bg-cream/[0.04] rounded-lg animate-pulse" />
            ))}
          </div>
        ))}
      </div>
...
function SidebarContent({ isOpen, onToggle, collapsed, onCollapseToggle, items }: SidebarProps) {
  ...
  return (
    <>
      ...
      >
        {/* ── Brand ── */}
        <div className={`flex items-center border-b border-cream/[0.07] flex-shrink-0 transition-all duration-300 ${collapsed ? 'justify-center px-0 py-[1.15rem]' : 'justify-between gap-3 px-5 py-[1.15rem]'}`}>
          {collapsed ? (
            <div className="w-8 h-8 rounded-lg bg-accent-orange flex items-center justify-center flex-shrink-0" title={fullName}>
        ...
        {collapsed && (
          <div className="flex justify-center py-3 flex-shrink-0">
            <Link
              ...
              className="w-8 h-8 flex items-center justify-center rounded-lg text-[#9e9690] hover:text-accent-orange hover:bg-accent-orange/[0.06] transition-all"
            >
        ...
        {/* ── Divider ── */}
        <div className="mx-3 h-px bg-cream/[0.07]" />

        {/* ── Navigation ── */}
        <nav
          className="flex-1 min-h-0 overflow-y-auto px-2 py-3 space-y-4"
          ...
          {CATEGORY_ORDER.map(category => {
            ...
            return (
              <div key={category}>
                ...
                {collapsed && <div className="mb-1.5 h-px bg-cream/[0.05] mx-1" />}
                ...
                <div className="space-y-0.5">
                  {catItems.map(item => {
                    ...
                    if (collapsed) {
                      // Icon-only mode
                      const navHref = hasChildren ? (item.children![0].href) : item.href;
                      return (
                        <div key={item.href} className="relative flex justify-center">
                          ...
                          <Link
                            href={navHref}
                            title={item.label}
                            className={`group w-9 h-9 flex items-center justify-center rounded-lg transition-all duration-150 ${
                              shouldHighlight
                    ...
                    return (
                      <div key={item.href}>
                        ...
                        {hasChildren && expanded && (
                          <div className="mt-0.5 ml-[22px] pl-3 space-y-0.5 border-l border-cream/[0.07]">
                            {item.children!.map(child => {
                              ...
                              return (
                                ...
                                >
                                  <span className={`w-[5px] h-[5px] rounded-full flex-shrink-0 ${childIsActive ? 'bg-accent-orange' : 'bg-[#4a4440]'}`} />
                                  {child.label}
                                </Link>
                              );
                            })}
        ...
        {/* ── Footer: user ── */}
        {!collapsed && (
          <div className="flex-shrink-0 px-4 py-3 border-t border-cream/[0.07]">
        ...
        {/* Collapse toggle tab — floats on the right edge, always visible, desktop only */}
        <button
          onClick={onCollapseToggle}
          title={collapsed ? 'Expand sidebar' : 'Collapse sidebar'}
          className="hidden md:flex absolute top-1/2 -translate-y-1/2 -right-[13px] w-[13px] h-10 items-center justify-center bg-[#16120e] border border-cream/[0.12] border-l-0 rounded-r-md text-[#5e5550] hover:text-accent-orange transition-colors z-40"
        >

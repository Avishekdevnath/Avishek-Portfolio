...
interface Props {
  bookmarkId: string;
  initialJD?: string;
  initialAnalysis?: BookmarkAIAnalysis | null;
}
...
export default function JobAnalysisTab({ bookmarkId, initialJD = '', initialAnalysis = null }: Props) {
  const [jd, setJd] = useState(initialJD);
  const [analysis, setAnalysis] = useState<BookmarkAIAnalysis | null>(initialAnalysis);
  ...
  const saveJD = async () => {
    setSavingJD(true);
    try {
      const res = await fetch(`/api/job-hunt/bookmarks/${bookmarkId}`, {
        method: 'PATCH',
  ...
  const generate = async () => {
    ...
    // Save JD first if dirty
    if (jdDirty) await saveJD();
    setGenerating(true);
    try {
      const res = await fetch(`/api/job-hunt/bookmarks/${bookmarkId}/analysis`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      });
      const data = await res.json();
    ...
    <div className="space-y-5">
      ...
      {analysis && (
        <div className="space-y-4">
          ...
          <Section
            title="Skill Gaps — Areas to Strengthen"
